...
class Table extends React.Component {
	...
	sortAlltime() {
		...
		if (this.state.alltimeOrder===0) {
			arr = this.state.arr.sort( (a,b) => a.alltime-b.alltime);
			this.setState({'alltimeOrder': 1});
		}
		else
		{
			arr = this.state.arr.sort( (a,b) => b.alltime-a.alltime);
			...
			<table>
				...
				<thead>
					<tr>
						<th>#</th>
						<th>Camper Name</th>
						<th onClick={this.sortRecent}>Points in past 30 days <i className="sort-arrow">{(this.state.recentOrder) ? '↑' : '↓'}</i></th>
						<th onClick={this.sortAlltime}>All time points <i className="sort-arrow">{(this.state.alltimeOrder) ? '↑' : '↓'}</i></th>
					</tr>
				</thead>
				<tbody>
						{this.state.rows}
